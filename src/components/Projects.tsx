...
// interface ProjectsProps { isAdmin: boolean; refreshSignal: number; }
...
//         ...p, id: p._id, github: p.githubUrl, live: p.liveUrl, category: p.category || "Web App"
//       }));
//       // Merge originals with database, removing duplicates
//       const merged = [...projectsData, ...formattedDb.filter(dbP => !projectsData.some(p => p.title === dbP.title))];
//       setAllProjects(merged);
//     } catch (e) {
//       setAllProjects(projectsData); // Fallback
//     } finally { setLoading(false); }
//   };

//   // 🚀 REFRESH TRIGGER: Runs loadData whenever refreshSignal changes
//   useEffect(() => { loadData(); }, [refreshSignal]);

//   const handleDelete = async (id: string) => {
//     if (!window.confirm("Delete this project permanently from MongoDB?")) return;
//     try {
//       await axios.delete(`http://localhost:5001/api/projects/${id}`);
//       loadData(); // Update list instantly
...
//         <h2 className="text-4xl font-extrabold text-gray-900 mb-12">My Creative Projects</h2>
//         <motion.div layout className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
//           <AnimatePresence mode="popLayout">
//             {allProjects.map((proj) => (
//               <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} key={proj.id} className="p-6 bg-white rounded-2xl border border-gray-150 shadow-sm hover:shadow-lg transition-all relative">
//                 <span className="text-[10px] text-gray-400 uppercase font-bold">{proj.category}</span>
//                 <h3 className="font-bold text-lg text-gray-900 mb-2">{proj.title}</h3>
//                 <p className="text-xs text-gray-500 mb-6 line-clamp-2">{proj.description}</p>
                
//                 {/* Delete button only for DB projects */}
//                 {isAdmin && typeof proj.id === 'string' && proj.id.length > 10 && (
//                   <button onClick={(e) => { e.stopPropagation(); handleDelete(proj.id); }} className="w-full py-2 mb-4 bg-red-50 text-red-600 rounded-xl text-[10px] font-bold border border-red-100 hover:bg-red-600 hover:text-white transition-all">
//                     <Trash2 className="w-3 h-3 inline mr-1" /> DELETE PROJECT
//                   </button>
//                 )}

//                 <div className="flex justify-between items-center text-[10px] font-bold text-gray-400 pt-4 border-t border-gray-50">
//                    <span className="text-gray-600">Explore Details ➔</span>
//                    <div className="flex gap-2"> <Github className="w-4.5 h-4.5" /> </div>
//                 </div>
//               </motion.div>
//             ))}
//           </AnimatePresence>
